...
- Lab data file path: labs/ot-ics-scada/data/events.jsonl
...
5. Data view pattern: ot_demo_*
6. Timestamp field: @timestamp
7. Save.

### Data ingestion options
Use your standard ingest method (Elastic Agent, file upload, Logstash, or API).
Import file: labs/ot-ics-scada/data/events.jsonl
Target index should match data view pattern: ot_demo_*

### Kibana validation
1. Go to Analytics then Discover.
2. Select data view: ot_demo_*
...
## 4) Query validation
### Splunk
1. Open file: labs/ot-ics-scada/splunk/starter-queries.spl
2. Copy IOC query and run.
3. Copy behavioral query and run.
4. Copy correlation query and run.

### ES|QL
1. Open file: labs/ot-ics-scada/elastic/esql/starter-queries.esql
...
### No events in Splunk
- Verify index name is exactly: ot_demo
- Widen time range
- Re-check ingestion source

### No events in Kibana
- Verify data view pattern is: ot_demo_*
