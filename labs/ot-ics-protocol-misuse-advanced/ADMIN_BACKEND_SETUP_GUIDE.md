...
- Lab data file path: labs/ot-ics-protocol-misuse-advanced/data/events.jsonl
...
3. Click Indexes.
4. Click New Index.
5. Create index named: ot_adv_demo
6. Save.
7. Go to Search and Reporting.
8. Open Add Data (or ask your Splunk admin to ingest JSONL file).
9. Upload file: labs/ot-ics-protocol-misuse-advanced/data/events.jsonl
...
5. Data view pattern: ot_adv_demo_*
6. Timestamp field: @timestamp
7. Save.

### Data ingestion options
Use your standard ingest method (Elastic Agent, file upload, Logstash, or API).
Import file: labs/ot-ics-protocol-misuse-advanced/data/events.jsonl
Target index should match data view pattern: ot_adv_demo_*

### Kibana validation
1. Go to Analytics then Discover.
2. Select data view: ot_adv_demo_*
...
## 4) Query validation
### Splunk
1. Open file: labs/ot-ics-protocol-misuse-advanced/splunk/starter-queries.spl
2. Copy IOC query and run.
3. Copy behavioral query and run.
4. Copy correlation query and run.

### ES|QL
1. Open file: labs/ot-ics-protocol-misuse-advanced/elastic/esql/starter-queries.esql
...
### No events in Splunk
- Verify index name is exactly: ot_adv_demo
- Widen time range
- Re-check ingestion source

### No events in Kibana
- Verify data view pattern is: ot_adv_demo_*
